                        ...
                        <div class="">
                            ...
                            <select name='idStagiaire' class="px-3 py-1 bg-gray-200 dark:bg-gray-600 rounded-lg">
                                @foreach($stagiaires as $stagiaire)
                                    <option value="{{$stagiaire->id}}" {{ $absence->stagiaire->id == $stagiaire->id ? "selected" : ""}}>{{$stagiaire->nom . " " . $stagiaire->prenom}}</option> 
                                @endforeach
                            </select>
                            @error('idStagiaire')
                        ...
                        <div class="">
                            ...
                            <select name='idSeance' class="px-3 py-1 bg-gray-200 dark:bg-gray-600 rounded-lg">
                                @foreach($seances as $seance)
                                    <option value="{{$seance->id}}" {{ $absence->seance->id == $seance->id ? "selected" : "" }}>{{$seance->nom}}</option>
                                @endforeach
                            </select>
                            @error('idSeance')
                                <div class="invalid-feedback">{{ $message }}</div>
